...
- [ ] the dry-run flag reaches `run_watch` and suppresses notification, proven
      by a test that asserts nothing was sent and that the watch still
      classified
- [ ] `GET /mail/events` returns only fields `redact_event` archives, and not
      every one of those: the archived message id is not returned, because
      this route answers without a token and a stable identifier is what that
      argument does not cover
- [ ] the archived field set itself is pinned, so widening it is a failing
