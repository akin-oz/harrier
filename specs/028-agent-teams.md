...
with a design that was wrong to choose, and this one carries twenty-seven
specs, a spec gate, commit trailers and CI resolution for a tool with one
user. Nothing in the governance is allowed to ask whether that is
...
Every member is `tools: Read, Glob, Grep, Bash`, read-only, reports
`file:line — what — fix` at P0/P1/P2, and declines findings belonging to
another lens.
...
|---|---|
| `review-principal-architect` | Twenty-seven specs, four guardians and a trailer gate for a single-user tool: proportionate, or paperwork? Several approved specs were corrected by their own implementation. |
...
- [x] every member is read-only in its frontmatter
...
Proven by services/api/tests/test_governance.py: membership in both
directions (`test_every_member_named_in_a_launch_document_exists`,
`test_every_team_agent_is_claimed_by_exactly_one_team`), read-only
frontmatter (`test_every_member_is_read_only`), the mirror
(`test_the_compiled_copy_matches_the_source`), and each readiness lens
citing a real failure
(`test_every_readiness_lens_cites_the_failure_that_motivated_it`).
...
`~/Documents/projects/sorrel` and `~/Documents/projects/blotato-comments`,
read for the pattern: a launch document that convenes the board, seed tasks
per member, read-only members with one lens each, and a lead that merges.
Both were read, neither modified.

The distinction this spec turns on is blotato's: the guardians check
compliance, the board exercises judgement, and a repository can be
...
work and needs the experimental flag. Changing any existing guardian. Any
